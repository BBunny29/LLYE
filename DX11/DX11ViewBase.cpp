#include "pch.h"
#include "DX11ViewBase.h"

DX11ViewBase::DX11ViewBase()
	: m_Width(0)
	, m_Height(0)
...
ID3D11ShaderResourceView* DX11ViewBase::GetShaderResourceViewRawPtr()
{
	return m_pSharderResourceView.Get();
}

ID3D11ShaderResourceView* const* DX11ViewBase::GetShaderResourceViewAddressOf()
{
	return m_pSharderResourceView.GetAddressOf();
}

Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> DX11ViewBase::GetShaderResourceView()
{
	return m_pSharderResourceView;
}

Microsoft::WRL::ComPtr<ID3D11Texture2D>& DX11ViewBase::GetTexture2D()
{
	return m_pRTVTexture2D;
}

const D3D11_VIEWPORT& DX11ViewBase::GetViewPort()
{
	return m_ViewPort;
}

HRESULT DX11ViewBase::CreateResource(
	...
	UINT cpuFlag)
{
	...
	try
	{
		...
		THROW_COM_ERROR_IF_FAILED(_hr, "RenderTargetBase에서 CreateTexute2D 생성 실패");
...
HRESULT DX11ViewBase::CreateTexture2D(ID3D11Device* pDevice, DXGI_FORMAT textureFormat, UINT bindFlags)
{
...
HRESULT DX11ViewBase::CreateTexture2D(ID3D11Device* pDevice, DXGI_FORMAT textureFormat, UINT bindFlags, D3D11_USAGE usage, UINT cpuFlag)
{
	D3D11_TEXTURE2D_DESC _textureDesc;
	_textureDesc.Width = static_cast<UINT>(m_Width);
	_textureDesc.Height = static_cast<UINT>(m_Height);
...
void DX11ViewBase::CreateViewPort()
{
