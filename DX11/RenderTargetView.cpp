	...
	DXGI_FORMAT spcificFormat)
{
	...
	try
	{
		...
		THROW_COM_ERROR_IF_FAILED(_hr, "RenderTargetView CreateResource Fail");

		_hr = CreateShaderResourceView(pDevice.Get(), shaderResourceFormat);

		THROW_COM_ERROR_IF_FAILED(_hr, "DepthStancilView, CreateSharderResourceView FAIL ");

		_hr = CreateRenderTargetView(pDevice.Get(), spcificFormat);

		THROW_COM_ERROR_IF_FAILED(_hr, "RenderTargetView CreateDepthStencil Fail");
...
void RenderTargetView::Initialize(Microsoft::WRL::ComPtr <ID3D11Device> pDevice, UINT width, UINT height, UINT mipMaps)
{
	...
	try
	{
		...
		THROW_COM_ERROR_IF_FAILED(_hr, "Create Textrue2D Fail");
		...
		THROW_COM_ERROR_IF_FAILED(_hr, "Create RenderTargetView Fail");
		...
		THROW_COM_ERROR_IF_FAILED(_hr, "Create ShaderResourceView Fail");
